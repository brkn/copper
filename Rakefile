...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("copper.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in copper.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    ...
    replace_in_file "copper.gemspec", /ruby_version = .*">= (.*)"/ => lowest_minor
